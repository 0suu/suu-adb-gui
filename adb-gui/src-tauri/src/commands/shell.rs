...
// Shell からは install/push/pull のような長時間コマンドも流せるため、
// 通常の対話コマンドより長めに待つ。
const SHELL_COMMAND_TIMEOUT_MS: u64 = 300_000;

#[tauri::command]
pub async fn run_adb_command(serial: String, args: Vec<String>) -> Result<CommandResult, String> {
    let args_refs: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
    match process_manager::exec_adb_for_device(&serial, &args_refs, SHELL_COMMAND_TIMEOUT_MS).await
    {
